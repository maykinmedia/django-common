...
easily parsed by machines and humans. Logging is one of the pilars of
:ref:`telemetry <otel>`.
...
Arguably the most important Django setting :-) Note that we assume the setup from
...
.. tip:: Disabling IP address logging is guaranteed to be GDPR-proof, though you should
   decide in your own project if you want to enable or disable this.
...
arguments. Prefer the past tense: ``thing_happened``, ``something_was_done``, and leave
all the particulars (what thing, what was done to whom) for the contextual parameters.
