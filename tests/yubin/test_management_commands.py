...
def test_retry_output():
    ...
    in_progress_message = Message.objects.create(
    ...
    in_progress_message.refresh_from_db()
    assert failed_message.status == Message.STATUS_QUEUED
    assert in_progress_message.status == Message.STATUS_IN_PROCESS


def test_delete_old_output():
    with time_machine.travel("2026-02-04"):
    ...
    with time_machine.travel("2026-06-01"):
